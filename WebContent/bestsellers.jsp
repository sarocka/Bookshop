...
<% List<Book> books =(List<Book>)session.getAttribute("bestsellers"); %>
<h1 style="font-family: Lucida Sans Unicode; margin-left: 20px"><i>Top 5</i></h1>

<table class="w3-table w3-striped">
<tr class="w3-sand"><th>Title</th><th>ISBN</th><th>Price</th><th>Cover</th><th>Rating</th><th>Publisher</th><th>Writer</th><th>&nbsp;</th></tr>
<% for (int i=books.size()-1; i>=books.size()-5; i--) {%>
	<tr><td><%=books.get(i).getTitle()%></td><td><%=books.get(i).getIsbn()%></td><td><%=books.get(i).getPrice()%>$</td>
	<td><%=books.get(i).getCover()%></td><td><span class="fa fa-star checked" style="color: gold"></span><%=books.get(i).getRating()%></td>
	<td><%=books.get(i).getPublisher()%></td><td><%=books.get(i).getWriter()%></td>
	<td><form action="AddToWishList">
	<input type="hidden" name="bookId" value="<%=books.get(i).getId()%>"/>
	<input class="addToWishList" type="submit" value="Add to wishlist"/></form></td></tr>
<%} %>
</table>
<div class="footer">
